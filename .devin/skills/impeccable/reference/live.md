...
1. `live.mjs`: boot. If the request names or implies a file, route, or app inside a monorepo, infer the concrete path and run `node .devin/skills/impeccable/scripts/live.mjs --target <path>` instead; then run the rest of this live session from the returned `projectRoot`.
2. Open the app URL that serves `pageFile` (infer from `package.json`, docs, terminal output, or an open tab). Never use `serverPort`; it's the helper, not the app. **devin:** `browser_navigate` to that URL before polling; do not skip. **Other harnesses:** use the available browser tool; if the URL is uncertain, ask the user once.
...
```bash
node .devin/skills/impeccable/scripts/live.mjs
```
...
```
LOOP:
  node .devin/skills/impeccable/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
**Stream mode (experimental, not for devin):**

```
node .devin/skills/impeccable/scripts/live-poll.mjs --stream   # stays running; one JSON line per event
  Handle event; run --reply in a separate command
  Repeat until "exit" line → Cleanup
```

Stream keeps one process alive and waits for `--reply` ack before polling again. Useful only when the harness reads incremental stdout reliably and quickly. **devin is not one of those:** background pattern notify on a long-running shell was ~5s to pick up events vs sub-second for one-shot exit notify. Default to one-shot everywhere unless you have measured otherwise.
...
```bash
node .devin/skills/impeccable/scripts/live-status.mjs
node .devin/skills/impeccable/scripts/live-resume.mjs --id SESSION_ID
node .devin/skills/impeccable/scripts/live-complete.mjs --id SESSION_ID
```
...
```bash
node .devin/skills/impeccable/scripts/live-insert.mjs --id EVENT_ID --count EVENT_COUNT --position after \
...
```bash
node .devin/skills/impeccable/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
```bash
node .devin/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node .devin/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
```bash
node .devin/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID steer_done ["Optional short note for a browser toast"]
```
...
```bash
node .devin/skills/impeccable/scripts/live-server.mjs stop
```
...
```bash
node .devin/skills/impeccable/scripts/detect-csp.mjs
```
