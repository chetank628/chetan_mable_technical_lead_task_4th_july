#!/usr/bin/env node
/**
 * Impeccable design hook — devin preToolUse write gate.
 *
 * devin's stop hook is not consistently dispatched by the headless agent, so
...
function devinBlockMessage(findings, filePath, config, cwd) {
...
function bumpdevinDenial(cache, sessionId, filePath, findings) {
  ...
  fileEntry.devinDenials = fileEntry.devinDenials && typeof fileEntry.devinDenials === 'object'
    ? fileEntry.devinDenials
    : {};
  fileEntry.devinDenials[key] = (fileEntry.devinDenials[key] || 0) + 1;
  return { key, count: fileEntry.devinDenials[key] };
}
...
async function main() {
  ...
  const message = appendDesignSystemNote(devinBlockMessage(filtered, filePath, config, cwd), scanOptions);
  const sessionId = event.session_id || event.conversation_id || 'unknown';
  const cache = readCache(cwd);
  const denial = bumpdevinDenial(cache, sessionId, filePath, filtered);
  ...
  if (denial.count > EDIT_COUNT_THRESHOLD) {
    ...
    return allow({
      ...
      devinDenialKey: denial.key,
      devinDenialCount: denial.count,
      downgraded: true,
