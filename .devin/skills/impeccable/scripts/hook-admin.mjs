...
const HOOK_MANIFEST_TARGETS = [
  ...
  {
    provider: '.devin',
    skillRel: '.devin/skills/impeccable',
    destRel: '.devin/hooks.json',
    manifest: () => ({
      version: 1,
      hooks: {
        preToolUse: [
          {
            command: 'node ".devin/skills/impeccable/scripts/hook-before-edit.mjs"',
            timeout: TIMEOUT_SECONDS,
          },
        ],
      },
...
function stripImpeccableHookEntry(entry) {
  ...
  // `command`/`args`: Claude/Codex/devin. `bash`/`powershell`: GitHub Copilot's
