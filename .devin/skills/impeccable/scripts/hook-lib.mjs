...
export function resolveTargetFiles(event, projectCwd) {
  ...
  // devin Write / StrReplace use `path`, not `file_path`.
...
export function resolveHarness(env = {}, event = null) {
  ...
  if (explicit === 'devin') return 'devin';
  ...
  if (typeof event?.conversation_id === 'string' && event.conversation_id) return 'devin';
  return 'claude';
}
...
function envProjectDir(fallback) {
  if (typeof process.env.devin_PROJECT_DIR === 'string' && process.env.devin_PROJECT_DIR) {
    return process.env.devin_PROJECT_DIR;
  }
  return fallback;
}
